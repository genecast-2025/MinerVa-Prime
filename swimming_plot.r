...
df_new_pid = read.table("new.PID.xls",header=T,sep="\t")
df_new_pid = df_new_pid[,c("PID","Individual_name")]

df_pid = read.table("../../input/20240519.summary.pid.xls",header=T,sep="\t")
dim(df_pid)
df_pid = merge(df_pid,df_new_pid)
dim(df_pid)
df_pid$PID = df_pid$Individual_name
df_pid = df_pid[,c("PID","MinerVa.Prime_longitudinal","EGFR_L858R_19DEL")]

df = read.table("../../input/20240519.summary.sample.xls",header=T,sep="\t")
dim(df)
df = merge(df,df_new_pid)
df$PID = df$Individual_name
dim(df)
df = merge(df,df_pid)
dim(df)
...
#df$re_DFS = factor(df$re_DFS,levels=c(0,1))
...
#df$metastasis_site
#df = subset(df,MinerVa.Prime_longitudinal == "Negative")
#dim(df)
#df = subset(df,re_DFS == 1)
#dim(df)

#df= subset(df,PID %in% c("P296428","P296336","P296414"))
# 提取唯一的患者信息
patient_info <- df %>%
  ...
  distinct() %>%
  mutate(ID = row_number())  # 添加一个数值ID方便排序
...
# 转换 simplex_ctDNA_ratio 列为数值类型，并处理 NA 值
#df$simplex_ctDNA_ratio <- as.numeric(as.character(df$simplex_ctDNA_ratio))
#df$simplex_ctDNA_ratio[is.na(df$simplex_ctDNA_ratio)] <- 0  # 将NA值设为0或其他合适的默认值
# 将 simplex_ctDNA_ratio 划分为10个分位区间，并转换为因子
#df$simplex_ctDNA_ratio_quantile <- cut(df$simplex_ctDNA_ratio, 
#                                       breaks = quantile(df$simplex_ctDNA_ratio, probs = seq(0, 1, by = 0.1), na.rm = TRUE),
#                                       include.lowest = TRUE, labels = FALSE)
#df$simplex_ctDNA_ratio_quantile <- factor(df$simplex_ctDNA_ratio_quantile)

#quantiles <- quantile(df$simplex_ctDNA_ratio, probs = seq(0, 1, by = 0.1), na.rm = TRUE)
#unique_quantiles <- unique(quantiles)
# 将 simplex_ctDNA_ratio 分成 10 个分位
#df$simplex_ctDNA_ratio_quantile <- cut(df$simplex_ctDNA_ratio, breaks = unique_quantiles, include.lowest = TRUE, labels = FALSE)


# 定义颜色映射函数
map_colors <- function(value, quantile) {
  if (value == "Positive") {
    #print(colorRampPalette(c("lightpink", "red"))(5)[as.numeric(quantile)])
    #return(colorRampPalette(c("lightpink", "red"))(5)[as.numeric(quantile)])
...
# 确保 fill_color 是一个因素，用于图例显示
#df$fill_color_factor <- factor(df$fill_color, levels = unique(df$fill_color))
df <- df %>%
  arrange(simplex_ctDNA_ratio_quantile) %>%
...
unique(df$simplex_ctDNA_ratio_quantile)
levels(df$fill_color_factor)
table(df[,c("simplex_ctDNA_ratio_quantile","fill_color_factor")])
...
swimming_plot <- ggplot(df) +
  ...
  #geom_point(data = df, aes(x = inter_FF, y = factor(PID, levels = unique(patient_info$PID)), size = simplex_ctDNA_ratio_quantile, fill = fill_color), shape = 21, color = "black") +
  #geom_point(data = df, aes(x = inter_FF, y = factor(PID, levels = unique(patient_info$PID)), fill = fill_color), shape = 21, size=4,color = "black") +
...
#  scale_fill_identity() +
#  scale_size_manual(name = "ctDNA Ratio Quantile", values = 1:10) +
#scale_fill_manual(name = "simplex_ctDNA_ratio_quantile", values = setNames(levels(df$fill_color_factor), levels(df$fill_color_factor))) +
#scale_fill_manual(
#    name = "simplex_ctDNA_ratio_quantile",
#    values = setNames(levels(df$fill_color_factor), levels(df$fill_color_factor)),
#    labels = as.character(unique(df$simplex_ctDNA_ratio_quantile))
#  ) +
